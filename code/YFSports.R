...
noahid <- "370.l.64399.t.10"
date.1 <- "2017-4-09"

test_run <- player_stats_dates(noahid,date.1,config)
###############################################################################################
player_stats_dates <- function(team_key,date,config){
  stat_url = paste0(ff_base,"/team/",noahid,"/roster;date=",date.1,"/players/stats;type=date;date=",date.1)
  stat_res <- GET(stat_url,config)
  stat_xml <- read_xml(as.character(stat_res)) %>% xml_ns_strip()#%>%xml_find_all("//full")
  player_xml <- xml_find_all(stat_xml,"//player") %>% xml_ns_strip()
  ...
  generate_player_stats <- function(player_xml){
    ...
    player_name <- xml_find_first(nx,"name/full") %>% xml_text
    player_key <- xml_find_first(nx,"player_key") %>% xml_text
    week <- xml_find_first(nx,"player_stats/date") %>% xml_integer()
    display_position <- xml_find_first(nx,"display_position") %>% xml_text
    stat_id <- xml_find_all(nx,"player_stats/stats/stat/stat_id") %>% xml_integer()
    stat_value <- xml_find_all(nx,"player_stats/stats/stat/value") %>% xml_text()
    return(data_frame(player_name = player_name,
...
new_test <- pull_daily_stats(jay,date.1,config)

week1_daily_stats <- group_by(team_df_old,team_key,team_name,Dates) %>% do(player_stats_dates(.$team_key,.$Dates,config)) %>% ungroup()
#######################################################################################################
...
team_df_old <- read_delim("/Users/noahknoblauch/Dropbox/Baseball/team_id.txt",delim = "\t")
...
week1 <- data_frame(Dates = seq(as.Date("2017-04-02"),as.Date("2017-4-09"), by = "day"))
week1 <- mutate(week1,c = 1)

team_df_old <- team_df_old %>% mutate(c = 1)
team_df_old <- inner_join(week1,team_df_old, by = "c")
